...
if __name__ == '__main__':
    ...
    user = "xxx@sina.com"           # 用户名
    passwd = "xxxx"                 # 口令
    receivers = ['xx@ruijie.com.cn', 'xxxxxx@qq.com']  # 接收列表

    ms=MailSender(host,user,passwd)
    ms.set_subject("title test")
    ms.set_mail_to(receivers)
    ms.set_content("content test")
    ms.set_attach("/home/test/HttpUtil.java",'application/text')
    ms.set_attach("/home/test/password.txt", 'application/octet-stream')
    ret=ms.send_mail()
    if ret is True:
        print "邮件发送成功"
    else:
